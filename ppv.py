...
# Add Baseball to allowed categories
...
async def fetch_schedule(page):
    try:
        ...
        res = await page.request.get(API_URL, headers=HEADERS, timeout=15000)
        if not res.ok:
            print(f"❌ Failed to fetch API JSON: HTTP {res.status}")
            return []

        data = await res.json()
        print(f"✅ Schedule fetched, found {len(data.get('streams', []))} categories")
...
async def scrape_streams(entries, browser):
    url_map = {}
...
async def main():
    async with async_playwright() as p:
        ...
        url_map = await scrape_streams(entries, browser)
