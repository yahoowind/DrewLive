...
def get_group_title(extinf_line):
    """
    Return a modified EXTINF line with the correct group-title if the line matches
    one of the allowed countries. Otherwise, return (None, None) to skip.
    """
    ...
    # No allowed country found in this EXTINF line — skip this channel
    return None, None


def unwrap_url(url):
    """
    Extract the actual URL from wrapped URLs if needed.
    """
...
def main():
    ...
    while i < len(lines):
        ...
        if line.startswith('#EXTINF'):
            ...
            # If channel does not belong to allowed groups, skip it
            if new_extinf_line is None:
                i += 2
                continue
    ...
    # Sort channels alphabetically by group name
