...
async def fetch_fstv_channels():
    async with async_playwright() as p:
        ...
        for url in MIRRORS:
            ...
            try:
                print(f"🌐 Trying {url}")
                # Attach route **just before** navigating
                page.route("**/*.m3u8**", lambda route, request: handle_request(request))
...
async def main():
    try:
        print("🚀 Starting FSTV scraping...")
        channels = await fetch_fstv_channels()
        if channels:
