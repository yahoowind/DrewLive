...
# Change timezone to Pacific Time (Los Angeles)
...
def parse_time_to_datetime_pt(day_time_str, base_date):
    """
    Parses a time string (e.g., "Sun 9:01 PM") and a base date to create a
    timezone-aware datetime object localized directly to Pacific Time (PT).
    """
    try:
        ...
        dt_pt = dt_naive.replace(tzinfo=PT)
        return dt_pt
    except Exception as e:
...
def format_xmltv_time(dt):
    """
    Formats a datetime object into the XMLTV required format,
    forcing the timezone offset to -0800 (PST).
    Example: 20250720190100 -0800
    """
...
def convert_to_xmltv(schedule, base_date):
    ...
    for prog in schedule:
        # Parse time using Pacific Time
        dt_pt = parse_time_to_datetime_pt(prog["start_time"], base_date)
        if not dt_pt:
            continue
    ...
    program_list.sort(key=lambda x: x["start_dt_pt"])
    ...
    xmltv.append('  <channel id="toonamiwest">')
    xmltv.append('    <display-name>Toonami Aftermath West</display-name>')
    xmltv.append('  </channel>')

    for i, prog in enumerate(program_list):
        start_dt = prog["start_dt_pt"]
        if i + 1 < len(program_list):
            stop_dt = program_list[i+1]["start_dt_pt"]
            if stop_dt <= start_dt:
        ...
        xmltv.append(f'  <programme channel="toonamiwest" start="{format_xmltv_time(start_dt)}" stop="{format_xmltv_time(stop_dt)}">')
...
async def main():
    ...
    if html:
        ...
        with open("toonamiwest-epg.xml", "wb") as f:
            f.write(xmltv_data)

        print("Saved XMLTV schedule to toonamiwest-epg.xml")
    else:
